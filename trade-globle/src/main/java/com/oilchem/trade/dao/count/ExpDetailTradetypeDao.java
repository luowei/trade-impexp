...
import com.oilchem.trade.domain.count.ExpDetailTradetype;
 ...
 */
public interface ExpDetailTradetypeDao extends PagingAndSortingRepository<ExpDetailTradetype, Long>,
        JpaSpecificationExecutor<ExpDetailTradetype> {

    @Modifying
    @Transactional
    @Query(value = " delete from " +
            " t_export_detail_tradetype " +
            "where " +
    ...
    @Query(value = " insert into  " +
            " t_export_detail_tradetype(col_year,col_month,year_month, product_code , product_name ,trade_type, num, unit , money, avg_price) " +
            "select " +
            ...
            "end as year_month ," +
            " product_code , product_name ,trade_type " +
            ...
            " where year_month= ?1 "+
            "group by  year_month,product_code,product_name,trade_type,unit  " ,nativeQuery = true)
