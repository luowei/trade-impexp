...
import com.oilchem.trade.domain.count.ExpDetailCompanytype;
 ...
 */
public interface ExpDetailCompanytypeDao extends PagingAndSortingRepository<ExpDetailCompanytype, Long>,
        JpaSpecificationExecutor<ExpDetailCompanytype> {

    @Modifying
    @Transactional
    @Query(value = " delete from " +
            " t_export_detail_companyType " +
            "where " +
    ...
    @Query(value = " insert into  " +
            " t_export_detail_companytype(col_year,col_month,year_month, product_code , product_name ,company_type, num, unit , money, avg_price) " +
            "select " +
            ...
            "round(sum(amount),2) as num, unit , " +
            "round(sum(amount_money),2) as money, " +
            "case sum(amount) " +
            "     when 0 then 0 " +
    ...
    @Transactional
    @Query(value = " insert into  " +
            " t_export_detail_companytype(col_year,col_month,year_month, product_code , product_name ,company_type,company_type, num, unit , money, avg_price) " +
            "select " +
            " convert(int,substring(ltrim(rtrim(year_month)), 1, 4)) as col_year, " +
            " convert(int,substring(ltrim(rtrim(year_month)), 6, 2)) as col_month, " +
            " case " +
